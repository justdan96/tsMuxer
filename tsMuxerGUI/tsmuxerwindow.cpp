...
QString fpsTextToFpsStr(const QString& fpsText)
{
	...
	if (p >= 0) {
		...
		return toFixedDecPoint(QString::number(myStrToFloat(leftStr) / myStrToFloat(rightStr), 'f', 3));
	}
	else
...
int TsMuxerWindow::findLangByCode(const QString& code)
{
	for (int i = 0; i < ui.langComboBox->count(); ++i) {
		char* addr = (char*) ui.langComboBox->itemData(i).toLongLong();
		if (addr && code == addr) 
			return i;
	}
	return 0;
...
QtvCodecInfo* TsMuxerWindow::getCurrentCodec()
{
	if (ui.trackLV->currentRow() == -1)
		return 0;
	long iCodec = ui.trackLV->item(ui.trackLV->currentRow(), 0)->data(Qt::UserRole).toLongLong();
	return iCodec ? (QtvCodecInfo*) (void*) iCodec : 0;
}
...
void TsMuxerWindow::onAudioSubtitlesParamsChanged()
{
    ...
    char* addr = (char*) ui.langComboBox->itemData(ui.langComboBox->currentIndex()).toLongLong();
    if (addr) 
        codecInfo->lang = addr;
    else
...
{
#ifdef WIN32
	OemToAnsi(arr.data(), (char*) arr.data());
#endif
	QString str = QString::fromLocal8Bit(arr);
	str.fromLocal8Bit(arr);
	...
	for (int i = 0; i < strList.size(); ++i)
	{
		...
		if (p >= 0) 
		{
			...
			if (tmpVal != 0 && runInMuxMode)
				muxForm.setProgress(tmpVal*10 + 0.5); // todo: uncomment here
		}
		else {
			if (runInMuxMode)
			{
