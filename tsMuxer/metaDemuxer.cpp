...
METADemuxer::~METADemuxer() {
	readClose();
	for (unsigned i = 0; i < m_iterators.size(); i++)
...

{
	...
	while (1)
	{
		...
		while (allDataDelayed) 
		{
            ...
            if (allDataDelayed) 
				for (unsigned i = 0; i < m_codecInfo.size(); i++) {
					ContainerToReaderWrapper* cReader = dynamic_cast <ContainerToReaderWrapper*> (m_codecInfo[i].m_dataReader);
					if (cReader)
        ...
        for (int i = 0; i < m_codecInfo.size(); ++i)
        {
            ...
            if (reader && reader->isSubStream()) 
            {
                if (!warned && (int) reader->getInsertSEI() != primarySEI) {
                    LTRACE(LT_INFO, 2, "Parameter 'insertSEI' for MVC dependent view differs from same param for AVC base view. Ignore and apply same value");
                    warned = true;
                }
...
int METADemuxer::addPGSubStream(const string& codec, const string& _codecStreamName, const map<string,string>& addParams, MPLSStreamInfo* subStream)
{
...
int METADemuxer::addStream(const string codec, const string& codecStreamName, const map<string,string>& addParams)
{
            ...
            for (int i = 0; i < mplsInfo.m_playItems.size(); ++i) 
            {
                ...
                if (isSubStream) {
                    if (mplsInfo.m_mvcFiles.size() == 0) {
                        THROW(ERR_INVALID_CODEC_FORMAT, "Current playlist file doesn't has MVC track info. Please, remove MVC track from the track list");
                    }
                    else if (mplsInfo.m_mvcFiles.size() <= i)
                        THROW(ERR_INVALID_CODEC_FORMAT, "Bad playlist file: number of CLPI files for AVC and VMC parts do not match");
	...
	if (fileList.size() > 1) 
	{
		...
		for (unsigned i = 0; i < fileList.size(); i++)
			listIterator->addFile(fileList[i]);
	}
	...
	else if (strEndWith(tmpname, ".ts") || strEndWith(tmpname, ".m2ts") || strEndWith(tmpname, ".mts") || strEndWith(tmpname, ".ssif"))
	{
		if (pid)
			dataReader = &m_containerReader;
		else
...
DetectStreamRez METADemuxer::DetectStreamReader(BufferedReaderManager& readManager, const string& fileName, bool calcDuration)
{
	AVChapters chapters;
	...
	if (demuxer)
	{
		...
		{
			StreamData& vect = itr->second;
			CheckStreamRez trackRez = detectTrackReader(vect.data(), vect.size(), 
				containerType, acceptedPidMap[itr->first].m_trackType, itr->first);
		...
		else if (strEndWith(tmpname,".pcm") || strEndWith(tmpname,".lpcm") || strEndWith(tmpname,".wav") || strEndWith(tmpname,".w64"))
...
CheckStreamRez METADemuxer::detectTrackReader(uint8_t* tmpBuffer, int len, 
											  AbstractStreamReader::ContainerType containerType,
...
AbstractStreamReader* METADemuxer::createCodec(const string& codecName, 
											   const map<string,string>& addParams,
											   const std::string& codecStreamName,
                                               const vector<MPLSPlayItem>& mplsInfo)
{
...
uint8_t* ContainerToReaderWrapper::readBlock(uint32_t readerID, uint32_t& readCnt, int& rez, bool* firstBlockVar)
{
	...
	if (demuxerData.m_firstRead) {
		for (map<uint32_t, DemuxerReadPolicy>::iterator itr = demuxerData.m_pids.begin(); itr != demuxerData.m_pids.end(); ++itr) 
		{
	...
	if ((readCnt > 0 && (policy == drpFragmented || 
	...
	else if (demuxerData.lastReadRez[pid] != AbstractReader::DATA_DELAYED || demuxerData.m_allFragmented)
	{
		...
		do {
			...
			demuxRez = demuxerData.m_demuxer->simpleDemuxBlock(demuxerData.demuxedData, demuxerData.m_pidSet, discardSize);
			for (DemuxedData::const_iterator itr = demuxerData.demuxedData.begin(); itr != demuxerData.demuxedData.end() && !m_terminated; ++itr)
			{
				if (itr->second.size() > MAX_DEMUX_BUFFER_SIZE)
					THROW(ERR_CONTAINER_STREAM_NOT_SYNC, "Reading buffer overflow. Possible container streams are not syncronized. Please, verify stream fps. File name: " << demuxerData.m_streamName);
			}
...
void ContainerToReaderWrapper::resetDelayedMark()
{
	...
	{
		DemuxerData& demuxerData = itr->second.m_demuxerData;
		for (map<uint32_t, uint32_t>::iterator itr2 = demuxerData.lastReadRez.begin(); itr2 != demuxerData.lastReadRez.end(); ++itr2)
										  ...
										  const CodecInfo* codecInfo)
{
	...
	if (codecInfo && (codecInfo->codecID == CODEC_S_PGS || codecInfo->codecID == CODEC_S_SUP  || codecInfo->codecID == CODEC_S_SRT))
