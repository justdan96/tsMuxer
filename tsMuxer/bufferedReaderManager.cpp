...
BufferedReaderManager::BufferedReaderManager ( uint32_t readersCnt, uint32_t blockSize, uint32_t allocSize, uint32_t prereadThreshold )
{
...
BufferedReaderManager::~BufferedReaderManager()
{
	for (size_t i = 0; i < m_fileReaders.size(); i++) {
		delete m_fileReaders[i]; // need to define destruction order first. This object MUST be deleted after MCVodStreamer
	}
}
