...
void SRTStreamReader::setAnimation(const text_subtitles::TextAnimation& animation)
{
			...
			m_charSize == 4 && *((uint32_t*)cur) == m_long_N) 
		{
			...
			if (cur - lastProcessedLine >= m_charSize)
				if (m_charSize == 1 && cur[-1] == '\r' ||
					m_charSize == 2 && ((uint16_t*)cur)[-1] == m_short_R ||
...
int SRTStreamReader::readPacket(AVPacket& avPacket)
{
	...
	if (rez == NEED_MORE_DATA) 
	{
		...
		if (renderedBuffer) 
		{
			m_dstSubCodec->setBuffer(renderedBuffer - MAX_AV_PACKET_SIZE, renderedLen, m_lastBlock && m_sourceText.size() == 0);
...
CheckStreamRez SRTStreamReader::checkStream(uint8_t* buffer, int len, 
											ContainerType containerType, 
											int containerDataType, 
											int containerStreamIndex)
{
	CheckStreamRez rez;
	if ((containerType == ctMKV || containerType == ctMOV) && containerDataType == TRACKTYPE_SRT || containerType == ctSRT) 
	{
...
std::string SRTStreamReader::detectUTF8Lang(uint8_t* buffer, int len)
{
	return "";
}
