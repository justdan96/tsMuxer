...
SingleFileMuxer::SingleFileMuxer(MuxerManager* owner):
    AbstractMuxer(owner),
    m_lastIndex(-1) 
{
}
...
void SingleFileMuxer::intAddStream(const std::string& streamName,
...
bool SingleFileMuxer::muxPacket(AVPacket& avPacket)
{
	...
	{
        const uint32_t blockSize = DEFAULT_FILE_BLOCK_SIZE;
		streamInfo->m_bufLen += avPacket.codec->writeAdditionData(streamInfo->m_buffer + streamInfo->m_bufLen, 
																  streamInfo->m_buffer + blockSize + MAX_AV_PACKET_SIZE + ADD_DATA_SIZE,
                                                                  avPacket, 0);
...
void SingleFileMuxer::parseMuxOpt(const std::string& opts)
{
	...
	for (int i = 0; i < params.size(); i++) {
		...
		if (paramPair[0] == "--split-duration") {
			LTRACE(LT_WARN, 2, "Warning! Splitting does not implemented for demux mode. Parameter " << paramPair[0] << " ignored.");
		}
