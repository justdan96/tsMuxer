	...
	while (totalReadedBytes < DETECT_STREAM_BUFFER_SIZE && lastReadRez != BufferedReader::DATA_EOF)
	{
		...
		for (curPos = data; curPos <= lastFrameAddr; curPos += TS_FRAME_SIZE)
		{
			if (!m2tsHdrDiscarded && m_m2tsMode) 
				curPos+=4;
			while (*curPos != 0x47 && curPos <= lastFrameAddr) 
				curPos++;
            ...
            */
			...
			{ // PMT
				...
				{
					memcpy(pmtBuffer+pmtBufferLen, curPos + tsPacket->getHeaderSize(), TS_FRAME_SIZE - tsPacket->getHeaderSize());
					...
					if (m_pmt.isFullBuff(pmtBuffer, pmtBufferLen)) {
						...
						for (PIDListMap::const_iterator itr = m_pmt.pidList.begin(); itr != m_pmt.pidList.end(); ++itr)
							trackList.insert(std::make_pair(itr->second.m_pid, 
							TrackInfo(itr->second.m_streamType, itr->second.m_lang, 0)));
...
int TSDemuxer::simpleDemuxBlock(DemuxedData& demuxedData, const PIDSet& acceptedPIDs, 
								int64_t& discardSize)
{
    if (m_firstDemuxCall) {
	...
	uint8_t* data = m_bufferedReader->readBlock(m_readerID, readedBytes, 
		            readRez, &isFirstBlock); // blocked read mode
	...
	if (isFirstBlock) 
	{
		if (m_curFileNum < m_mplsInfo.size()) 
			m_prevFileLen += (m_mplsInfo[m_curFileNum].OUT_time - m_mplsInfo[m_curFileNum].IN_time) * 2; // in 90Khz clock
		else
	...
	if (m_pmt.pidList.size() == 0 || mvcContinueExpected()) {
		...
		for (m_curPos = data; m_curPos <= lastFrameAddr; m_curPos += TS_FRAME_SIZE)
		{
			if (!forpmtm2tsHdrDiscarded && m_m2tsMode) 
				m_curPos+=4;
			while (*m_curPos != 0x47 && m_curPos <= lastFrameAddr) 
				m_curPos++;
			...
			{ // PMT
				if (tsPacket->payloadStart || pmtBufferLen > 0)
				{
					memcpy(pmtBuffer+pmtBufferLen, m_curPos + tsPacket->getHeaderSize(), TS_FRAME_SIZE - tsPacket->getHeaderSize());
					pmtBufferLen += TS_FRAME_SIZE - tsPacket->getHeaderSize();
					if (m_pmt.isFullBuff(pmtBuffer, pmtBufferLen)) {
						m_pmt.deserialize(pmtBuffer, pmtBufferLen);
                        if (m_pmt.video_type != STREAM_TYPE_VIDEO_MVC)
	...
	for (m_curPos = data; m_curPos <= lastFrameAddr; m_curPos += TS_FRAME_SIZE)
	{
		...
		bool pesStartCode = frameData[0] == 0 && frameData[1] == 0 && 
							frameData[2] == 1 && tsPacket->payloadStart;
		if (pesStartCode) {
			...
			{ // 2880 - ac3 frame len
                ...
                */
				...
				if (streamInfo != m_pmt.pidList.end() && 
					             isVideoPID(streamInfo->second.m_streamType) && 
			...
			if (streamInfo != m_pmt.pidList.end() && streamInfo->second.m_streamType != STREAM_TYPE_SUB_PGS) // demux PGS with PES headers
...
uint64_t TSDemuxer::getDemuxedSize()
{
