...
double pgs_frame_rates[16] = {0, 23.97602397602397, 24, 25, 29.97002997002997, 0, 50, 59.94005994005994, 0, 0, 0, 0, 0, 0, 0, 0};
...
int PGSStreamReader::readObjectDef(uint8_t* pos, uint8_t* end)
{
	...
	if (m_needRescale) {
		decodeRleData(composition_object_horizontal_position[object_id], composition_object_vertical_position[object_id]);
...
void PGSStreamReader::renderTextShow(int64_t inTime)
{
    ...
    const int64_t PLANEINITIALIZATIONTIME = 90000.0 * (m_scaled_width*m_scaled_height) / PIXEL_COMPOSITION_RATE + 0.999;
	...
	// composition segment. pts=x,  dts = x-0.0648 (pts alignment to video grid) (I get constant value from real PGS track as example)
	int rLen = m_render->composePresentationSegment(curPos, CM_Start, inTime, inTime - PRESENTATION_DTS_DELTA, m_objectWindowTop, m_demuxMode, m_forced_on_flag); 
	...
	rLen = m_render->composeWindowDefinition(curPos, inTime - windowsTransferTime, inTime - PRESENTATION_DTS_DELTA, m_objectWindowTop, m_objectWindowHeight, m_demuxMode);
	m_renderedBlocks.push_back(PGSRenderedBlock(inTime - windowsTransferTime, inTime - PRESENTATION_DTS_DELTA, rLen, curPos));
	curPos += rLen;
	
	
	// palette.             pts=x-0.0648, dts = x-0.0648
	rLen = m_render->composePaletteDefinition(m_render->m_paletteByColor, curPos, inTime - PRESENTATION_DTS_DELTA, inTime - PRESENTATION_DTS_DELTA, m_demuxMode);
	m_renderedBlocks.push_back(PGSRenderedBlock(inTime - PRESENTATION_DTS_DELTA, inTime - PRESENTATION_DTS_DELTA, rLen, curPos));
	curPos += rLen;
	// object               pts=x-0.0627, dts = x-0.0648
    //inTime - 5643
    int64_t odfPTS = inTime - PRESENTATION_DTS_DELTA + compositionDecodeTime;
	rLen = m_render->composeObjectDefinition(curPos, odfPTS, inTime - PRESENTATION_DTS_DELTA, m_render->minLine(), m_render->maxLine(), m_demuxMode);
...
void PGSStreamReader::renderTextHide(int64_t outTime)
{
	...
	int rLen = m_render->composePresentationSegment(curPos, CM_Finish, outTime, outTime - windowsTransferTime - 90, m_objectWindowTop, m_demuxMode, false);
	...
	rLen = m_render->composeWindowDefinition(curPos, outTime - windowsTransferTime, outTime - windowsTransferTime - 90, m_objectWindowTop, m_objectWindowHeight, m_demuxMode);
	m_renderedBlocks.push_back(PGSRenderedBlock(outTime - windowsTransferTime, outTime - windowsTransferTime - 90, rLen, curPos));
	curPos += rLen;
...
int PGSStreamReader::readPacket(AVPacket& avPacket)
{
	...
	if (m_video_height == 0) 
	{
		...
		LTRACE(LT_INFO, 2, "Decoding PGS stream (track " << m_streamIndex << "): " << " Resolution: " << m_video_width << ':' << m_video_height << "  Frame rate: " << m_frame_rate);
		if (m_newFps != 0 && fabs(m_newFps - m_frame_rate) > 1e-4)
		{
			LTRACE(LT_INFO, 2, "Change FPS from " << m_frame_rate << " to " << m_newFps << " for PGS stream #" << m_streamIndex);
			m_scale =  m_frame_rate / m_newFps;
		}
		if (m_needRescale) 
			LTRACE(LT_INFO, 2, "Change PGS resolution from " << m_video_width << ':' << m_video_height << " to " << m_scaled_width << ':' << m_scaled_height << 
	...
	if (pesStartCode) 
	{
		...
		if (pesPacket->flagsLo & 0x80) { // pts exists
...
int PGSStreamReader::flushPacket(AVPacket& avPacket)
{
	return 0;
}
...
CheckStreamRez PGSStreamReader::checkStream(uint8_t* buffer, int len, 
											...
											int containerStreamIndex)
{
	...
	if (m_video_height != 0) {
		...
		ostr << " Resolution: " << m_video_width << ':' <<
						   m_video_height << " Frame rate: " << m_frame_rate;
...
int PGSStreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd, AVPacket& avPacket, PriorityDataInfo* priorityData)
{
	if (!m_demuxMode)
		return 0;
