...
MPEGRawDataHeader::MPEGRawDataHeader(int maxBufferLen):
                   MPEGHeader(), 
				   m_headerIncludedToBuff(0), 
				   m_data_buffer_len(0), 
				   m_max_data_len(maxBufferLen)
{
	if (maxBufferLen > 0)
...
MPEGRawDataHeader::~MPEGRawDataHeader() {
...
uint32_t MPEGPictureHeader::getPictureSize()
{
	return m_headerSize + m_data_buffer_len;
}
	...
	switch (cnt) {
		...
		case 5:
			switch(reader.getBit()) {
				...
				case 0:
					switch(reader.getBit()) { // last bit of first byte
						case 1: 
