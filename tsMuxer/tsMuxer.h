...
private:
	...
	std::vector< std::pair<uint8_t*, int> > m_m2tsDelayBlocks; // postpone M2TS PCR processing (fill previous data on next PCR after several PES packets)
	...
	struct StreamInfo {
		StreamInfo(){ m_pts = m_dts = ULLONG_MAX; m_tsCnt = 1;}
		int64_t m_pts;
		int64_t m_dts;
		int m_tsCnt;
	};
