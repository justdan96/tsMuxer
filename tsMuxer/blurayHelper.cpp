...
uint8_t bdIndexData[] =  {
    ...
    0x00, 0x00, 0x01, 0x92, 0x00, 0x00, 0x00, 0x18,
...
uint8_t bdMovieObjectData[] = 
{
    ...
    0x00, 0x03, 0x80, 0x00, 0x00, 0x05, 0x50, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00,
    0x00, 0x00, 0x50, 0x40, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x40, // here 5040
    
};
...
bool BlurayHelper::writeBluRayFiles(bool usedBlackPL, int mplsNum, int blankNum, bool stereoMode) 
{
    ...
    if (m_dt == DT_BLURAY) {
        ...
        bdIndexData[0x96] = (byte)HDR10_metadata[0]; // HDR flags
...
bool BlurayHelper::createCLPIFile(TSMuxer* muxer, int clpiNum, bool doLog)
{
    ...
    for (unsigned i = 0; i < muxer->splitFileCnt(); i++)
    {
        ...
        dstDir = string("BDMV") + getDirSeparator() + string("BACKUP") + getDirSeparator() + string("CLIPINF") + getDirSeparator();
        if (!file->open((prefix + dstDir + clipName + string(".clpi")).c_str(), File::ofWrite))
        {
            delete [] clpiBuffer;
            delete file;
...
bool BlurayHelper::createMPLSFile(TSMuxer* mainMuxer, TSMuxer* subMuxer,
                    int autoChapterLen, vector<double> customChapters,
                    DiskType dt, int mplsOffset,
                    bool isMvcBaseViewR)
{
    ...
    if (subMuxer) {
        ...
        for (PIDListMap::const_iterator itr = pidListMVC.begin(); itr != pidListMVC.end(); ++itr) {
            ...
            data.type = 2; // Identify an elementary stream of the Clip used by a SubPath with SubPath_type set to 2,3,4,5,6,8 or 9
    ...
    if (!file->open((prefix + dstDir + strPadLeft(int32ToStr(mplsOffset),5,'0') + string(".mpls")).c_str(), File::ofWrite))
        return false;
    ...
    dstDir = string("BDMV") + getDirSeparator() + string("BACKUP") + getDirSeparator() + string("PLAYLIST") + getDirSeparator();
    if (!file->open((prefix + dstDir + strPadLeft(int32ToStr(mplsOffset),5,'0') + string(".mpls")).c_str(), File::ofWrite)) {
        delete file;
        return false;
    }
...
string BlurayHelper::m2tsFileName(int num)
{
    ...
    return prefix + string("BDMV") + separator + string("STREAM") + separator + strPadLeft(int32ToStr(num),5, '0') + string(".m2ts");
}
...
string BlurayHelper::ssifFileName(int num)
{
    string prefix = m_isoWriter ? "" : m_dstPath;
    char separator = m_isoWriter ? '/' : getDirSeparator();
    return prefix + string("BDMV") + separator + string("STREAM") + separator + string("SSIF") + separator + strPadLeft(int32ToStr(num),5, '0') + string(".ssif");
}

IsoWriter* BlurayHelper::isoWriter() const
{
    return m_isoWriter;
}
...
bool BlurayHelper::isVirtualFS() const
{
    return m_isoWriter != 0;
}
