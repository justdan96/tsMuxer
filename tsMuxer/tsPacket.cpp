...
{
    return stream_coding_type==STREAM_TYPE_VIDEO_MPEG2 || 
        stream_coding_type==STREAM_TYPE_VIDEO_H264 || 
        stream_coding_type==STREAM_TYPE_VIDEO_VC1 ||
        stream_coding_type==STREAM_TYPE_VIDEO_MVC ||
        stream_coding_type==STREAM_TYPE_VIDEO_H265;
}

bool isAudioStreamType(int stream_coding_type)
{
    return stream_coding_type==0x80 || stream_coding_type==0x81 ||
...
TS_program_map_section::TS_program_map_section(): video_pid(0), audio_pid(0), sub_pid(0), pcr_pid(0), program_number(0),
						casID(0), casPID(0)
{
...
uint32_t TS_program_map_section::serialize(uint8_t* buffer, int max_buf_size, bool addLang)
{
	...
	int sectionLen =  9 +  (video_pid ? 5 : 0) + (audio_pid ? 5 : 0)  + 
		(sub_pid ? 5 : 0) + (pidList.size() * 5) + 4 + program_info_len;
	...
	bitWriter.putBits(8, 0xfc); // scenarist: 0xfc, prev example: 0x84          here               1 0 000 1 00               1 1 111 1 00
	...
	for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end(); ++itr)
	{
		...
		if (*itr->second.m_lang && addLang) 
        {
			...
			for (int k = 0; k < 3; k++)
				bitWriter.putBits( 8, itr->second.m_lang[k]); // lang code[i]
			bitWriter.putBits(8, 0);
		}
	}
...
void CLPIParser::parseProgramInfo(uint8_t* buffer, uint8_t* end, 
                                  std::vector<CLPIProgramInfo>& programInfoMap,
...
void CLPIParser::composeProgramInfo(BitStreamWriter& writer, bool isSsExt)
{
	...
	//for (int i=0; i < number_of_program_sequences; i++) 
	{
        ...
        int streams = 0;
        for (std::map<int, CLPIStreamInfo>::const_iterator itr = m_streamInfo.begin(); 
...
void CLPIParser::CPI_SS(uint8_t* buffer, int dataLength)
{
    parseCPI(buffer, buffer + dataLength);
}
...
void CLPIParser::composeExtentInfo(BitStreamWriter& writer)
{
    ...
    lengthPos[1] = my_htonl(writer.getBitsCount()/8 - beforeCount); // data_block_start_address, same as extentStart point start
...
void MPLSParser::SubPath_extension(BitStreamWriter& writer)
{
    ...
    for (int i = 0; i < pmtIndexList.size(); i++)
        composeSubPlayItem(writer, i, 0, pmtIndexList);
...
void MPLSParser::composeSubPath(BitStreamWriter& writer, int subPathNum, std::vector<PMTIndex>& pmtIndexList, int type)
{
	...
	writer.putBits(8, type); // SubPath_type = 7 (In-mux and Synchronous type of Picture-in-Picture), 5 - MVC depend stream
...
void MPLSParser::composeSubPlayItem(BitStreamWriter& writer, int playItemNum, int subPathNum, std::vector<PMTIndex>& pmtIndexList)
{
	uint16_t* lengthPos = (uint16_t*) (writer.getBuffer() + writer.getBitsCount()/8);
	writer.putBits(16,0); // skip lengthField
	int beforeCount = writer.getBitsCount()/8;
...
int MPLSParser::composePip_metadata(uint8_t* buffer, int bufferSize, std::vector<PMTIndex>& pmtIndexList)
{
	...
	for (int i = 0; i < pmtIndexList.size(); ++i)
	{
		for (int k = 0; k < pipStreams.size(); k++) 
        {
			...
			writer.putBits(8, 0); // reserved_for_future_use
            writer.putBits(4, pipParams.lumma >= 0 ? 1 : 0); // pip_timeline_type == 1. Synchronous type of Picture-in-Picture 
            ...
            writer.putBits(8,0); // reserved_for_future_use 8 bslbf
			if (pipParams.lumma >= 0) { // is_luma_key==1b
		...
		*(lengthPos+1) = my_htonl(writer.getBitsCount()/8 - initPos + 4); // data_block_start_address
		for (int i = 0; i < extDataBlockInfo.size(); i++) 
		{
			*(extDataStartAddrPos[i]) = my_htonl(writer.getBitsCount()/8 - initPos + 4);
			for (int j = 0; j < extDataBlockInfo[i].data.size(); ++j)
    ...
    for (int i = 0; i < streamInfo.size(); i++)
    {
        ...
        if (isAudioStreamType(stream_coding_type) && streamInfo[i].isSecondary) 
        {
            ...
            if (number_of_secondary_video_stream_entries == 0) {
                writer.putBits(8, number_of_primary_audio_stream_entries); // number_of_primary_audio_ref_entries allowed to join with this stream
...
void M2TSStreamInfo::blurayStreamParams(double fps, bool interlaced, int width, int height, float ar,
                   int *video_format, int *frame_rate_index, int *aspect_ratio_index)
{
    *video_format = 0;
    *frame_rate_index = 0;
...
MPLSStreamInfo::MPLSStreamInfo(const MPLSStreamInfo& other):
    M2TSStreamInfo(other)
{
    type = other.type;
    character_code = other.character_code;
