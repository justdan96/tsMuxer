...
const static int  MAX_TRACK_SIZE = (MAX(MAX(sizeof(MatroskaVideoTrack), 
...
int MatroskaDemuxer::ebml_read_ascii (uint32_t *id, char **str)
{
    ...
    if ((res = ebml_read_element_id(id, NULL)) < 0 ||
        (res = ebml_read_element_length(&rlength)) < 0)
        return res;
    size = rlength;
        ...
        switch (id) {
            ...
            case EBML_ID_EBMLREADVERSION: {
                ...
                if ((res = ebml_read_uint(&id, &num)) < 0)
                    return res;
                if (num > EBML_VERSION) {
                    THROW(ERR_MATROSKA_PARSE, "EBML version " << num << " > " << 
						  EBML_VERSION << " is not supported");
            ...
            case EBML_ID_EBMLMAXSIZELENGTH: {
                ...
                if (num > sizeof(uint64_t)) {
                    THROW(ERR_MATROSKA_PARSE,  "Integers of size " << num << " (> " << sizeof(uint64_t) <<
						                       ") not supported");
                }
                break;
            }
...
int MatroskaDemuxer::matroska_parse_block(uint8_t *data, int size,
            ...
            for (slice=0; slice<slices; slice++) {
                ...
                if (tracks[track]->encodingAlgo == COMPRESSION_STRIP_HEADERS)
                {
                    ...
                    if (offset) {
                        ...
                        memcpy(curPtr, &tracks[track]->encodingAlgoPriv[0], offset); // place extra header direct to data
                    }
                }
...
int MatroskaDemuxer::matroska_parse_blockgroup (uint64_t cluster_time)
{
    ...
    if (size > 0)
        res = matroska_parse_block(data, size, pos, cluster_time,
...
int MatroskaDemuxer::ebml_read_skip ()
{
    ...
    if ((res = ebml_read_element_id(&id, NULL)) < 0 ||
        (res = ebml_read_element_length(&length)) < 0)
        return res;
    skip_bytes(length);
    return 0;
}
    ...
    while (res == 0) {
        ...
        switch (id) {
            ...
            case MATROSKA_ID_SIMPLEBLOCK:
                ...
                res = ebml_read_binary(&id, &data, &size);
                if (res == 0)
                    res = matroska_parse_block(data, size, pos,
            ...
            case MATROSKA_ID_CLUSTER:
                return 0; // I don't know why here is the next cluster without level up. Jim send me it. Probably file error
            default:
...
int MatroskaDemuxer::ebml_read_num(int max_size, uint64_t *number)
{
    ...
    if (!(total = get_byte())) {
        ...
        if (!m_isEOF) 
			THROW(ERR_MATROSKA_PARSE, "Matroska parse error: Invalid EBML number size " << total << " at pos " << m_processedBytes-1);
        return -BufferedReader::DATA_EOF;
    }
...
int MatroskaDemuxer::ebml_read_date (uint32_t *id, int64_t *date)
{
  return ebml_read_sint(id, date);
}
...
int MatroskaDemuxer::ebml_read_float (uint32_t *id, double *num)
{
    ...
    if (size == 4) {
        ...
        *num= av_int2dbl(get_be64());
        ...
        offset_t pos = m_processedBytes;
        THROW(ERR_MATROSKA_PARSE,
               "Invalid float element size " << size << " at position " << pos);
    }
    return 0;
}
...
int MatroskaDemuxer::ebml_read_utf8 (uint32_t *id, char **str)
{
  return ebml_read_ascii(id, str);
}
...
int MatroskaDemuxer::matroska_parse_tracks ()
{
    ...
    while (res == 0) {
        ...
        switch (id) {
            ...
            default:
                LTRACE(LT_INFO, 0, 
                       "Unknown entry " << id << " in track header");
                /* fall-through */
        ...
        switch (id) {
            ...
            case MATROSKA_ID_TRACKTYPE: {
                ...
                switch (track->type) {
                    ...
                    default:
                        LTRACE(LT_INFO, 0,
                               "Unknown or unsupported track type " << track->type);
                    ...
                    switch (id) {
                        ...
                        case MATROSKA_ID_VIDEOSTEREOMODE: {
                            ...
                            if (num != MATROSKA_EYE_MODE_MONO &&
                                ...
                                LTRACE(LT_INFO, 0,
                                       "Ignoring unknown eye mode " << (uint32_t) num);
                                break;
                            }
                        ...
                        case MATROSKA_ID_VIDEOASPECTRATIO: {
                            ...
                            if (num != MATROSKA_ASPECT_RATIO_MODE_FREE &&
                                ...
                                LTRACE(LT_INFO, 0,
                                       "Ignoring unknown aspect ratio " << (uint32_t) num);
                                break;
                            }
                        ...
                        default:
                            LTRACE(LT_INFO, 0,
                                   "Unknown video track header entry " << id <<
                                   " - ignoring\n");
                            /* pass-through */
            ...
            case MATROSKA_ID_TRACKAUDIO: {
                ...
                while (res == 0) {
                    ...
                    switch (id) {
                        ...
                        default:
                            LTRACE(LT_INFO, 0,
                                   "Unknown audio track header entry " << id <<
                                   " - ignoring\n");
                            /* pass-through */
            ...
            default:
                LTRACE(LT_INFO, 0,
                       "Unknown track header entry " << id << " - ignoring");
                /* pass-through */
...
int MatroskaDemuxer::simpleDemuxBlock(DemuxedData& demuxedData, 
