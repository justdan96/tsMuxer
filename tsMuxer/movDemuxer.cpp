...
{
public:
	MovParsedAudioTrackData(MovDemuxer* demuxer, MOVStreamContext* sc): ParsedTrackPrivData(), m_demuxer(demuxer), m_sc(sc) 
	{
		isAAC = false;
	}
		...
		for (; left > 4; ++i) {
			...
			if (m_sc->sample_size == 0) {
				if (m_sc->m_indexCur + i >= m_sc->m_index.size())
					THROW(ERR_MOV_PARSE, "Out of index for AAC track #" << m_sc->ffindex << " at position "  << m_demuxer->getProcessedBytes());
				left -= m_sc->m_index[m_sc->m_indexCur + i];
			}
		}
		if (left < 0 || left > 4)
			THROW(ERR_MOV_PARSE, "Invalid AAC frame for track #" << m_sc->ffindex << " at position " << m_demuxer->getProcessedBytes());
		if (!isAAC)
			i = 0;
...
{
public:
    MovParsedH264TrackData(MovDemuxer* demuxer, MOVStreamContext* sc): ParsedTrackPrivData(), m_demuxer(demuxer), m_sc(sc), nal_length_size(4)
    {
		...
		for(; spsCnt > 0; spsCnt--) 
		{
            ...
            if (nalSize > 0) {
			    ...
			    for (int i = 0; i < nalSize; ++i, ++src)
				    spsPpsList.rbegin()->push_back(*src);
            }
		}
		int ppsCnt = *src++;
		for(; ppsCnt > 0; ppsCnt--) 
	...
	int newBufferSize(uint8_t* buff, int size) override 
	{
		...
		while (buff < end) {
			if (buff + nal_length_size > end)
				THROW(ERR_MOV_PARSE, "MP4/MOV error: Invalid H.264/AVC frame at position " << m_demuxer->getProcessedBytes());
			uint32_t nalSize = getNalSize(buff);
			buff += nal_length_size;
			if (buff + nalSize > end)
 				THROW(ERR_MOV_PARSE, "MP4/MOV error: Invalid H.264/AVC frame at position " << m_demuxer->getProcessedBytes());
        ...
        int spsPpsSize = 0;
        for (int i = 0; i < spsPpsList.size(); ++i)
            spsPpsSize += spsPpsList[i].size() + 4;
...
{
public:
	MovParsedSRTTrackData(MovDemuxer* demuxer, MOVStreamContext* sc): ParsedTrackPrivData(), m_demuxer(demuxer), m_sc(sc) {
		m_packetCnt = 0;
		sttsPos = 0;
		m_timeOffset = 0;
	}
	...
	int64_t getSttsVal() {
		if (sttsCnt == 0) {
			...
			if (sttsPos >= m_sc->stts_data.size())
				THROW(ERR_MOV_PARSE, "MP4/MOV error: invalid stts index for SRT track #" << m_sc->ffindex << " at position " << m_demuxer->getProcessedBytes());
...
void MovDemuxer::getTrackList(std::map<uint32_t,TrackInfo>& trackList)
{
	for (int i = 0; i < num_tracks; i++) {
		if (tracks[i]->type != TRACK_TYPE_CONTROL)
			trackList.insert(std::make_pair(i+1,TrackInfo(tracks[i]->type == TRACK_TYPE_SUBTITLE ? TRACKTYPE_SRT : 0, tracks[i]->language, 0)));
	}
}
...
int MovDemuxer::mov_read_cmov(MOVAtom atom)
{
	THROW(ERR_MOV_PARSE, "Compressed MOV not supported in current version");
}
...
int MovDemuxer::mov_read_wide(MOVAtom atom)
{
    ...
    if (atom.size < 8)
        return 0; // continue 
    if (get_be32() != 0) {// 0 sized mdat atom... use the 'wide' atom size 
...
int MovDemuxer::mov_read_trun(MOVAtom atom)
{
    ...
    flags = get_be24();
    entries = get_be32();
    if (flags & 0x001) data_offset        = get_be32();
    if (flags & 0x004) first_sample_flags = get_be32();
    ...
    for (i = 0; i < entries; i++) {
        ...
        if (flags & 0x100) sample_duration = get_be32();
        if (flags & 0x200) sample_size     = get_be32();
        if (flags & 0x400) sample_flags    = get_be32();
        if (flags & 0x800) 
        {
        ...
        if ((keyframe = st->type == TRACK_TYPE_AUDIO ||
...
int MovDemuxer::mov_read_tfhd(MOVAtom atom)
{
    ...
    if (flags & 0x01) frag->base_data_offset = get_be64();
    else              frag->base_data_offset = frag->moof_offset;
    if (flags & 0x02) frag->stsd_id          = get_be32();
...
int MovDemuxer::mov_read_tkhd(MOVAtom atom)
{
    return 0;
}
...
int MovDemuxer::mov_read_stss(MOVAtom atom)
{
    ...
    if(entries >= UINT_MAX / sizeof(int))
        return -1;
    for(int i=0; i < entries; i++)
        st->keyframes.push_back(get_be32());
    return 0;
}
...
int64_t MovDemuxer::getFileDurationNano() const
{
    return fileDuration;
}
...
int MovDemuxer::mov_read_stco(MOVAtom atom)
{
    ...
    if  (atom.type == MKTAG('s','t','c','o'))
        for(int i = 0; i < entries; i++)
			sc->chunk_offsets.push_back(get_be32());
    else if (atom.type == MKTAG('c','o','6','4'))
        for(int i = 0; i < entries; i++)
            sc->chunk_offsets.push_back(get_be64());
    else
        return -1;
...
int MovDemuxer::mov_read_smi(MOVAtom atom)
{
    return 0;
}
...
int MovDemuxer::mov_read_wave(MOVAtom atom)
{
	...
	*/
	if (atom.size > 8) { // to read frma, esds atoms 
        if (mov_read_default(atom) < 0)
            return -1;
    } else
        skip_bytes(atom.size);
...
void MovDemuxer::setFileIterator(FileNameIterator* itr)
{
    m_fileIterator = itr;
}
