...
void ByteFileWriter::writeDString(const std::string& value, int len)
{
...
{
    return m_fileType == FileType_File || m_fileType == FileType_RealtimeFile;
}
...
void FileEntryInfo::writeEntity(ByteFileWriter& writer, FileEntryInfo* subDir)
{
    ...
    writer.writeLE8(!subDir->isFile() ? 0x02 : (isSystemFile ? 0x10 : 0));  // File Characteristics, 'directory' bit (1-th)
...
void FileEntryInfo::serializeDir()
{
    ...
    writer.writeLongAD(0x800, m_parent ? m_parent->m_sectorNum : m_owner->absoluteSectorNum(), 0x01, 0); // parent entry ICB
    writer.writeLE16(0); // Length of Implementation Use  
    ...
    // ------------ 2 (entries) ---------------
    for (int i = 0; i < m_files.size(); ++i)
        writeEntity(writer, m_files[i]);
    for (int i = 0; i < m_subDirs.size(); ++i)
        writeEntity(writer, m_subDirs[i]);
...
bool IsoWriter::open(const std::string& fileName, int64_t diskSize, int extraISOBlocks)
{
    ...
    for (int i = currentFileSizeLBN; i < requiredFileSizeLBN; ++i)
        m_file.write(m_buffer, SECTOR_SIZE);
...
bool IsoWriter::createInterleavedFile(const std::string& inFile1, const std::string& inFile2, const std::string& outFile)
{
...
void IsoWriter::allocateMetadata()
{
    ...
    if (m_systemStreamLBN + 3 > m_metadataFileLen / SECTOR_SIZE)
        throw std::runtime_error("ISO error: Not enough space in metadata partition. It possible in a split mode if a lot of files. Please provide addition input parameter --extra-iso-space to increate metadata space. Default value for this parameter in split mode: 4");
...
void IsoWriter::close()
{
    ...
    m_tagLocationBaseAddr = m_partitionStartAddress;
    writeExtentFileDescriptor(FileType_MetadataMirror, m_metadataFileLen, m_metadataMirrorLBN - m_partitionStartAddress, 0);
    
    // allocate space for metadata mirror file
    memset(m_buffer, 0, sizeof(m_buffer));
    for (int i = 0; i < m_metadataFileLen/SECTOR_SIZE; ++i)
        m_file.write(m_buffer, SECTOR_SIZE);
...
void IsoWriter::writeDescriptors()
{
    ...
    writeAnchorVolumeDescriptor(m_partitionEndAddress + ALLOC_BLOCK_SIZE/SECTOR_SIZE); // add space for last 64K anchor volume
...
{
    return m_file.pos() / SECTOR_SIZE - m_tagLocationBaseAddr;
}
...
{
    dir->serialize();
    for (int i = 0; i < dir->m_files.size(); ++i)
        writeEntity(dir->m_files[i]);
    for (int i = 0; i < dir->m_subDirs.size(); ++i)
        writeEntity(dir->m_subDirs[i]);
}
...
int IsoWriter::allocateEntity(FileEntryInfo* entity, int sectorNum)
{
    ...
    for (int i = 0; i < entity->m_files.size(); ++i)
        sectorNum = allocateEntity(entity->m_files[i], sectorNum);
    for (int i = 0; i < entity->m_subDirs.size(); ++i)
        sectorNum = allocateEntity(entity->m_subDirs[i], sectorNum);
    return sectorNum;
}
...
int IsoWriter::writeExtentFileDescriptor(uint8_t fileType, uint64_t len, uint32_t pos, int linkCount, ExtentList* extents)
{
    int sectorsWrited = 0;
