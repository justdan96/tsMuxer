...
int NALUnit::decodeNAL2(uint8_t* srcBuffer, uint8_t* srcEnd, uint8_t* dstBuffer, size_t dstBufferSize, bool* keepSrcBuffer)
{
    uint8_t* initDstBuffer = dstBuffer;
    uint8_t* srcStart = srcBuffer;
    *keepSrcBuffer = true;
...
void HRDParams::resetDefault(bool mvc)
{
    ...
    if (!mvc) {
        ...
        cpb_size_value_minus1[0] = 234374; // 234374*128 = 30 mbit per frame max. It is max allowed value for Blu-ray disks
    }
    else {
        bit_rate_scale = 2;
        cpb_size_scale = 3;
    ...
    for(int i = 0; i <= num_level_values_signalled_minus1; i++ ) 
    {
        ...
        for(int j = 0; j <= num_applicable_ops_minus1[ i ]; j++ ) 
        {
            ...
            for(int k = 0; k <= dummy; k++ )
                extractUEGolombCode(); // applicable_op_target_view_id[ i ][ j ][ k ]
...
{
    return nal_unit_type == nuSliceIDR || 
           nal_unit_type == nuSliceExt && !non_idr_flag;
}
...
int SliceUnit::deserialize(uint8_t* buffer, uint8_t* end, 
...
void SEIUnit::sei_payload(SPSUnit& sps, int payloadType, uint8_t* curBuff, int payloadSize, int orig_hrd_parameters_present_flag)
{
	if( payloadType  ==  0 )
...
void SEIUnit::serialize_buffering_period_message(const SPSUnit& sps, BitStreamWriter& writer, bool seiHeader)
{
	...
	if (sps.nalHrdParams.isPresent) { // NalHrdBpPresentFlag
		for(int SchedSelIdx = 0; SchedSelIdx <= sps.nalHrdParams.cpb_cnt_minus1; SchedSelIdx++ ) 
		{
			writer.putBits(sps.nalHrdParams.initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay[SchedSelIdx]);
			writer.putBits(sps.nalHrdParams.initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay_offset[SchedSelIdx]);
		}
	}
	if (sps.vclHrdParams.isPresent) { // NalHrdBpPresentFlag
		for(int SchedSelIdx = 0; SchedSelIdx <= sps.vclHrdParams.cpb_cnt_minus1; SchedSelIdx++ ) 
		{
			writer.putBits(sps.vclHrdParams.initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay[SchedSelIdx]);
			writer.putBits(sps.vclHrdParams.initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay_offset[SchedSelIdx]);
		}
	}
	write_byte_align_bits(writer);
	// ---------
        ...
        if (!operation_point_flag )
        {
            ...
            if( !all_view_components_in_au_flag ) 
            {
                ...
                for(int i = 0; i <= num_view_components_minus1; i++ )
                   bitReader.getBits(10);  // sei_view_id[ i ]
            }
        } 
        else {
