...
void TextSubtitlesRenderWin32::drawText(const std::wstring& text, RECT* rect)
{
    ...
    path.AddString(text.c_str(), text.length(), &fontFamily, m_font.m_opts & 0xf, m_font.m_size, Gdiplus::Point(rect->left, rect->top), &strformat );
...
void TextSubtitlesRenderWin32::flushRasterBuffer()
{
	GdiFlush();
}
