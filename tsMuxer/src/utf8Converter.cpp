    ...
    std::wstring toWideString(uint8_t* start, size_t widesize, SourceFormat srcFormat)
    {
		...
		else if (srcFormat == sfDefault) 
		{
			if (cd == 0)
			{
				...
				if (cd == (iconv_t)-1)
					THROW(ERR_COMMON, "Can't initialize iconv library for source encoding " << sourceEncoding);
			}
			...
			switch (srcFormat) {
				...
				case sfUTF8:
		            ...
		            *targetstart = 0;
					break;
				case sfUTF16be:
					 for (uint16_t* ptr = (uint16_t*) start; ptr < (uint16_t*) sourceend; ++ptr)
						 *ptr = my_ntohs(*ptr);
				case sfUTF16le:
					...
					break;
				case sfUTF32be:
					 for (uint32_t* ptr = (uint32_t*) start; ptr < (uint32_t*) sourceend; ++ptr)
						 *ptr = my_ntohl(*ptr);
				case sfUTF32le:
					res = ConvertUTF32toUTF16((const UTF32**)&sourcestart, (const UTF32*) sourceend, &targetstart, targetend, strictConversion);
		            *targetstart = 0;
					break;
			}
			...
			switch (srcFormat) {
				...
				case sfUTF16be:
					 for (uint16_t* ptr = (uint16_t*) start; ptr < (uint16_t*) sourceend; ++ptr)
						 *ptr = my_ntohs(*ptr);
				case sfUTF16le:
					res = ConvertUTF16toUTF32((const UTF16**)&sourcestart, (const UTF16*) sourceend, &targetstart, targetend, strictConversion);
		            *targetstart = 0;
					break;
				case sfUTF32be:
					 for (uint32_t* ptr = (uint32_t*) start; ptr < (uint32_t*) sourceend; ++ptr)
