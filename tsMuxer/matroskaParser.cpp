		...
		for (int i = 0; i < spsCnt; i++) {
            ...
            if (spsLen > 0) {
			    ...
			    for (int j = 0; j < spsLen; j++)
				    curData.push_back(bitReader.getBits(8));
            }
		}
...
ParsedSRTTrackData::ParsedSRTTrackData(uint8_t* buff, int size): ParsedTrackPrivData(buff, size)
{
	m_packetCnt = 0;
}
