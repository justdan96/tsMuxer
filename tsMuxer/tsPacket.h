...
struct TSPacket {
		...
		*/
		...
		inline uint16_t getPID() {
			return (uint16_t) (PIDHi << 8) + PIDLow;
		}
		...
		inline unsigned getHeaderSize() {
			return TS_HEADER_SIZE  +  (afExists ? adaptiveField.length + 1 : 0);
		}
...
{
    M2TSStreamInfo(): streamPID(0), character_code(0), width(0), height(0), frame_rate_index(3), video_format(0),
		isSecondary(false) {}
    ...
    static void blurayStreamParams(double fps, bool interlaced, int width, int height, float ar,
...
private:
	void parseProgramInfo(uint8_t* buffer, uint8_t* end,
                          std::vector<CLPIProgramInfo>& programInfo,
...
private:
	void composeSubPlayItem(BitStreamWriter& writer, int playItemNum, int subPathNum, std::vector<PMTIndex>& pmtIndexList);
