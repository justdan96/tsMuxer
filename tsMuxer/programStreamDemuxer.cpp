...
uint64_t ProgramStreamDemuxer::getDemuxedSize()
{
...
int ProgramStreamDemuxer::mpegps_psm_parse(uint8_t* buff, uint8_t* end)
{
	...
	if (psm_length > MAX_PES_HEADER_SIZE) {
		THROW(ERR_COMMON, "Can't parse Program Stream Map. Too large size " << psm_length << ". Max allowed size 1018 bytes.");
	}
...
long ProgramStreamDemuxer::processPES(uint8_t* buff, uint8_t* end, int& afterPesHeader)
{
    ...
    if (!((startcode >= 0xc0 && startcode <= 0xdf) ||
          (startcode >= 0xe0 && startcode <= 0xef) ||
	...
	if (pesPacket->flagsLo & 0x02) // PES_CRC_flag 1 bslbf
		curBuf+=2;
	if (pesPacket->flagsLo & 0x01) {  // PES_extension_flag 1 bslbf
    ...
    if (startcode == PES_PRIVATE_DATA1 && m_psm_es_type[startcode] == 0) 
	{
		afterPesHeader++;
        startcode = *curBuf++;
        if (startcode >= 0xb0 && startcode <= 0xbf) { // MLP/TrueHD audio has a 4-byte header, keep header
		}			
        else if (startcode >= 0xa0 && startcode <= 0xaf) { // PCM
...
bool ProgramStreamDemuxer::isVideoPID(uint32_t pid)
{
	return pid >= 0x55 && pid <= 0x5f || // vc1
		   ...
		   m_psm_es_type[pid & 0xff] == STREAM_TYPE_VIDEO_H264 || 
           m_psm_es_type[pid & 0xff] == STREAM_TYPE_VIDEO_MVC;
}

//static uint64_t prevDts = 0;
//static int ggCnt = 0;

int ProgramStreamDemuxer::simpleDemuxBlock(DemuxedData& demuxedData, 
										   ...
										   int64_t& discardSize)
{
	...
	while (curBuf <= end - 9)
	{
		...
		if ((startcode >= 0xc0 && startcode <= 0xef) ||
			  (startcode == PES_PRIVATE_DATA1) || (startcode == PES_VC1_ID) || 
				...
				if ((pesPacket->flagsLo & 0x80) == 0x80) 
				{
					...
					if (startcode == 85) 
					{
						...
						LTRACE(LT_INFO, 2, "Cnt=" << ++ggCnt << "  PTS: " << pesPacket->getPts() << 
							...
							int32ToStr(afterPesData[2],16) << int32ToStr(afterPesData[3],16));
						prevDts = tmpDts;
					}
					*/
				}
...
int64_t getLastPCR(File& file, int bufferSize, int64_t fileSize)
{
    ...
    while (curPtr <= bufEnd - 9) {
        ...
        if ((startcode >= 0xc0 && startcode <= 0xef) ||
            (startcode == PES_PRIVATE_DATA1) || (startcode == PES_VC1_ID) || (startcode == PES_PRIVATE_DATA2)) 
        {
            if ((pesPacket->flagsLo & 0x80) == 0x80)
