...
public:
	struct DemuxerData 
	{
		...
		DemuxerData() {m_demuxer = 0; m_firstRead = true; m_iterator = 0; m_allFragmented = true;}
		bool m_firstRead;
...
private:
	...
	AbstractStreamReader* createCodec(const std::string& codecName, 
		                              const std::map<std::string,std::string>& addParams,
	...
	static std::string findBluRayFile(const std::string& streamDir, const std::string& requestDir, const std::string& requestFile);
	const std::vector<MPLSParser> getMplsInfo(const std::string& mplsFileName);

    int addPGSubStream(const std::string& codec, const std::string& _codecStreamName, const std::map<std::string,std::string>& addParams, 
                        MPLSStreamInfo* subStream);
