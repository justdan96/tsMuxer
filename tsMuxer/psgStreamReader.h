...
private:
	...
	{
		PGSRenderedBlock(int64_t _pts, int64_t _dts, int _len, uint8_t* _data): pts(_pts), dts(_dts), len(_len), data(_data) {} 
		int64_t pts;
		int64_t dts;
		int len;
		uint8_t* data;
	...
	enum State {stParsePES, stParsePGS, stAVPacketFragmented};
	enum CompositionState {csNormalCase, csAcquisitionPoint, csEpochStart, csEpochContinue};
	State m_state;
	uint8_t* m_curPos;
	uint8_t* m_buffer;
	int m_tmpBufferLen;
