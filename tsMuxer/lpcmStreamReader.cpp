...
uint32_t LPCMStreamReader::convertWavToPCM(uint8_t* start, uint8_t* end)
{
	...
	int ch1FullSize = (end-start)/m_channels;
	// 1. convert byte order to little endian 
	if (m_bitsPerSample == 16) {
		for (uint16_t* curPos = (uint16_t*)start; curPos < (uint16_t*) end; ++curPos)
	...
	if (m_needPCMHdr) {
		int audio_data_payload_size = (m_bitsPerSample==20?24:m_bitsPerSample) * m_freq * ((m_channels+1)&0xfe) / 8 / 200; // 5 ms frame len. 1000/5 = 200
...
uint32_t LPCMStreamReader::convertLPCMToWAV(uint8_t* start, uint8_t* end)
{
	...
	return (end-start) - (m_channels%2 == 1 ? ch1FullSize : 0);
}
...
int LPCMStreamReader::decodeWaveHeader(uint8_t* buff, uint8_t* end)
{
	...
	if (m_channels == 0) 
	{
		...
		if (m_freq != 48000 && m_freq != 96000 && m_freq != 192000)
			THROW (ERR_COMMON, "Sample rate " << m_freq << " is not supported for LPCM format. Allowed values: 48000, 96000, 192000");
		m_bitsPerSample = waveFormatPCMEx->wBitsPerSample;
		if (m_bitsPerSample != 16 && m_bitsPerSample != 20 && m_bitsPerSample != 24)
			THROW (ERR_COMMON, "Bit depth " << m_bitsPerSample << " is not supported for LPCM format. Allowed values: 16bit, 20bit, 24bit");
		m_lfeExists = false;
		if (waveFormatPCMEx->wFormatTag == WAVE_FORMAT_EXTENSIBLE)
		{
			...
			if (m_channels > 2) {
				if (m_channels == 3) {
					...
					LTRACE(LT_WARN, 2, "Warning! Multi channels WAVE file for stream " << m_streamIndex << " do not contain channels configuration info. Applying default value: L R BL BR");
				}
				else if (m_channels == 5) {
					LTRACE(LT_WARN, 2, "Warning! Multi channels WAVE file for stream " << m_streamIndex << " do not contain channels configuration info. Applying default value: L R C BL BR");
				}
				else if (m_channels == 6) {
					LTRACE(LT_WARN, 2, "Warning! Multi channels WAVE file for stream " << m_streamIndex << " do not contain channels configuration info. Applying default value: L R C LFE BL BR");
					m_lfeExists = true;
				}
				else if (m_channels == 7) {
					LTRACE(LT_WARN, 2, "Warning! Multi channels WAVE file for stream " << m_streamIndex << " do not contain channels configuration info. Applying default value: L R C BL BR SL SR");
				}
				else if (m_channels == 8) {
					LTRACE(LT_WARN, 2, "Warning! Multi channels WAVE file for stream " << m_streamIndex << " do not contain channels configuration info. Applying default value: L R C LFE BL BR SL SR");
					m_lfeExists = true;
				}
			}
		}
...
int LPCMStreamReader::decodeFrame(uint8_t* buff, uint8_t* end, int& skipBytes, int& skipBeforeBytes)
{
	...
	else if (m_headerType == htWAVE || m_headerType == htWAVE64)
	{
		...
		//if (m_curChankLen == 0 || (buff[0] == 'R' && buff[1] == 'I' && buff[2] == 'F' && buff[3] == 'F') || (buff[0] == 'r' && buff[1] == 'i' && buff[2] == 'f' && buff[3] == 'f'))
        const uint32_t* curPtr32 = (const uint32_t*) buff;
        if (m_curChankLen == 0 && ( *curPtr32 == RIFF_SMALL || *curPtr32 == RIFF_LARGE))
		{
            if (end-buff < 8)
                return NOT_ENOUGHT_BUFFER;
            if (!m_openSizeWaveFormat || curPtr32[1] == 0)
            {
			    ...
			    if (hdrSize == NOT_ENOUGHT_BUFFER)
				    return NOT_ENOUGHT_BUFFER;
			    skipBeforeBytes = hdrSize;
                if (m_needSync)
                    m_curChankLen = 0; // decode header again
            }
		}
		if (m_frameRest == 0) {
            ...
            if (m_curChankLen)
                frameLen = FFMIN(reqFrameLen, m_curChankLen);
...
int LPCMStreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd, AVPacket& avPacket, PriorityDataInfo* priorityData)
{
	uint8_t* curPos = dstBuffer;
	if (m_demuxMode) {
		if (m_firstFrame) {
			if (dstEnd - dstBuffer < sizeof(WAVEFORMATPCMEX) + 8)
				THROW(ERR_COMMON, "LPCM stream error: Not enought buffer for writing headers");
			// write wave header
...
void LPCMStreamReader::setHeadersType(LPCMStreamReader::LPCMHeaderType value)
{
    m_headerType = value;
}

bool LPCMStreamReader::detectLPCMType(uint8_t* buffer, int len)
{
	if (len == 0)
		return false;
	...
	while (curPos < end)
	{
		...
		if (frameLen != 960 && frameLen != 1920 && frameLen != 2880 && frameLen != 3840
			&& frameLen != 1440 && frameLen != 4320 && frameLen != 5760 && frameLen != 7680
			&& frameLen != 8640 && frameLen != 11520 && frameLen != 17280) {
				break;
		}
		curPos += frameLen + 4;
	}
...
int LPCMStreamReader::flushPacket(AVPacket& avPacket)
{
	...
	{
		int samplesRest = m_frameRest/m_channels;
...
const CodecInfo& LPCMStreamReader::getCodecInfo() 
{ 
    return lpcmCodecInfo; 
}
