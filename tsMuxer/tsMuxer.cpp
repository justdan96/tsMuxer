...
void TSMuxer::intAddStream(const std::string& streamName,
...
*/
	...
	if (codecName == "V_MPEG4/ISO/AVC") {
		...
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_VIDEO_H264, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
    else if (codecName == "V_MPEG4/ISO/MVC") {
        H264StreamReader* h264Reader = (H264StreamReader*) codecReader;
        m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_VIDEO_MVC, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
    }
    else if (codecName == "V_MPEGH/ISO/HEVC") {
        m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_VIDEO_H265, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
    }
	else if (codecName == "V_MS/VFW/WVC1") {
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_VIDEO_VC1, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else if (codecName == "V_MPEG-2") {
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_VIDEO_MPEG2, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else if (codecName == "A_AAC") 
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_AUDIO_AAC, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	else if (codecName == "S_SUP") 
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_SUB_PGS, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	else if (codecName == "S_HDMV/PGS") 
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_SUB_PGS, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	else if (codecName == "S_TEXT/UTF8") 
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_SUB_PGS, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	else if (codecName == "A_AC3") 
    {
        ...
        m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(streamType, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else if (codecName == "A_MP3") {
		MpegAudioStreamReader* mp3Reader = (MpegAudioStreamReader*) codecReader;
		if (mp3Reader->getLayer() >= 2)
			m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_AUDIO_MPEG2, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
		else
			m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_AUDIO_MPEG1, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else if (codecName == "A_LPCM") {
		m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(STREAM_TYPE_AUDIO_LPCM, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else if (codecName == "A_DTS") {
        ...
        m_pmt.pidList.insert(std::make_pair(tsStreamIndex, PMTStreamInfo(audioType, tsStreamIndex, descrBuffer, descriptorLen, codecReader, lang,isSecondary)));
	}
	else
...
int TSMuxer::calcM2tsFrameCnt()
{
	...
	for (int i = 0; i < m_m2tsDelayBlocks.size(); i++) 
		byteCnt += m_m2tsDelayBlocks[i].second;
...
void TSMuxer::buildPesHeader(int pesStreamID, AVPacket& avPacket, int pid)
{
	...
	int additionDataSize = avPacket.codec->writeAdditionData(tmpBuffer + pesPacket->getHeaderLength(), 
                                                             tmpBuffer + sizeof(tmpBuffer), avPacket,
                                                             &tmpPriorityData);
    ...
    for (int i = 0; i < tmpPriorityData.size(); ++i)
        m_priorityData.push_back(std::pair<int,int>(tmpPriorityData[i].first + pesPacket->getHeaderLength(), tmpPriorityData[i].second));
}
...
std::string TSMuxer::getNextName(const std::string curName) 
{
	...
	if (m_bluRayMode) {
		...
		m_curFileNum++;
		string fileExt = extractFileExt(m_origFileName);
		result = m_origFileName.substr(0, m_origFileName.size()-fileExt.size()-1) + ".split." + int32ToStr(m_curFileNum) + "." + fileExt;
	}
...
void TSMuxer::gotoNextFile(uint64_t newPts) 
{
	...
	for (PIDListMap::iterator itr = m_pmt.pidList.begin(); itr != m_pmt.pidList.end(); ++itr) 
	{
		...
		if (pmtInfo.m_index[spSize-2].size() > 0) {
			...
			pmtInfo.m_index[spSize-1].insert(make_pair(shortPts, PMTIndexData(firstDataFrame, lastPesSize)));
			pmtInfo.m_index[spSize-2].erase(pmtInfo.m_index[spSize-2].rbegin()->first);
		}
        */
	}
		...
		if (m_computeMuxStats && (pesPacket->flagsLo & 0x80) == 0x80) 
		{
			...
			if (vCodec && m_pesIFrame) {
                ...
                if (m_pesSpsPps || !SPSRequired) 
                {
                    ...
                    {
				        curIndex.insert(std::make_pair(curPts, PMTIndexData(m_muxedPacketCnt[m_muxedPacketCnt.size()-1], 0)));
                        updateIdx = true;
                    }
                }
			...
			//else if (vCodec) {
			//	PMTIndex& curIndex = *streamInfo.m_index.rbegin();
			//	curIndex.insert(std::make_pair(curPts, PMTIndexData(m_muxedPacketCnt[m_muxedPacketCnt.size()-1], m_pesDataSize)));
			//}
			else if (aCodec) {
...
bool TSMuxer::blockFull() const
{
    return m_processedBlockSize >= m_interliaveBlockSize;
}
    ...
    */
	...
	if (m_firstPts[m_firstPts.size()-1] == -1 /*|| avPacket.pts < m_firstPts[m_firstPts.size()-1]*/)
	{
		if (m_firstPts[m_firstPts.size()-1] == -1) {
			...
			if (m_fixed_pcr_offset < 0)
				m_fixed_pcr_offset = 0;
			m_prevM2TSPCR = (m_fixed_pcr_offset-100) * 300; // add a little extra delta (100) to prevent 0 bitrate for first several packets
		}
    ...
    else if (newPES && m_interliaveBlockSize > 0 && m_canSwithBlock && (blockFull() || m_sublingMuxer && m_sublingMuxer->blockFull()))
    {
		...
		if (m_cbrBitrate != -1 && m_lastPCR != -1) {
			...
			if (newPCR - m_lastPCR >= m_pcr_delta && m_lastPCR != -1) 
			{
				...
				if (m_lastPESDTS != -1 && m_lastPCR > m_lastPESDTS) {
					LTRACE(LT_ERROR, 2, "VBV buffer overflow at position " << (m_lastPCR - m_fixed_pcr_offset)/90000.0 << " sec");
				}
			}
		}
...
vector<int64_t> TSMuxer::getFirstPts() {
	...
	for (int i = 0; i < m_firstPts.size(); i++)
		rez.push_back(nanoClockToPts(m_firstPts[i]) + m_timeOffset);
	return rez;
}
...
vector<int64_t> TSMuxer::getLastPts() {
	std::vector<int64_t> rez;
	for (int i = 0; i < m_lastPts.size(); i++)
		rez.push_back(nanoClockToPts(m_lastPts[i]) + m_timeOffset);
    //if (!rez.empty())
...
bool TSMuxer::isInterleaveMode() const
{
    return m_masterMode || m_subMode;
}

std::vector<int32_t> TSMuxer::getInterleaveInfo(int idx) const
{
    return m_interliaveInfo[idx];
}
